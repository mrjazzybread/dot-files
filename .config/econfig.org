...
#+end_src
* Window Management
Resizes the active window according to the gold ratio (extremely dope).
#+begin_src emacs-lisp :results none
...
Hide the mode line in X windows
#+begin_src emacs-lisp :results none
...
#+begin_src emacs-lisp :results none
  (start-process-shell-command
   "feh" nil
   "feh --bg-fill ~/.config/bg/lain.jpeg --bg-fill ~/.config/bg/lain.jpeg")
#+end_src
...
** Multiple monitors
I truly never know man
#+begin_src emacs-lisp :results none
  (setq exwm-randr-workspace-monitor-plist '(0 "HDMI-1-0"))
  (add-hook 'exwm-randr-screen-change-hook
            (lambda ()
              (start-process-shell-command
               "xrandr" nil "xrandr --output HDMI-1-0 --left-of eDP-1 --auto")))
  (exwm-randr-mode 1)
#+end_src
** Keybindings
Reset EXWM
...
#+begin_src emacs-lisp :results none
  (add-hook 'window-size-change-functions #'rotate:main-vertical)
#+end_src
