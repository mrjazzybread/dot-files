...
(setq org-todo-keywords
      '((sequence "APPOINTMENT(p)" "SPEC(s)" "TODO(t)" "IN-PROGRESS(i@/!)" "VERIFYING(v!)" "BLOCKED(b@)" "👁️👄👁️(q)" "|" "DONE(d!)" "OBE(o@!)" "WONT-DO(w@/!)" )
        ))
;; TODO colors
...
Fixed size for Vertico posframe
#+begin_src emacs-lisp :results none
...
Replace I-search with consult line
#+begin_src emacs-lisp :results none
  (global-set-key (kbd "C-s") #'consult-line)
#+end_src

Do not use posframe for consult line
#+begin_src emacs-lisp :results none
  (setq vertico-multiform-commands
        '((consult-line (:not posframe))
  	(switch-to-buffer (:not nerd-icons-icon-for-buffer))
          (t posframe)))
#+end_src

When the cursor is hovering over a string that can be interpreted as a
file path, ~find-file~ tries to find it.
#+begin_src emacs-lisp :results none
