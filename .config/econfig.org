...
#+begin_src emacs-lisp
  (modify-all-frames-parameters '((font . "Roboto Mono 22")))
  (modify-all-frames-parameters '((background-color . "#34293E")))
#+end_src
...
We add special rules for the minor and major modes that are allowed to
perform splits on the current window layout.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (keymap-set corfu-popupinfo-map "C-M-i" #'corfu-popupinfo-toggle)
...
#+begin_src emacs-lisp
  (use-package tuareg
    :vc (:url "https://github.com/mrjazzybread/tuareg"
  	    :rev :newest))
  #+end_src
...
* ~Dired~
By default, deleting a file removes it from your file system.  This
makes it so that deleting files moves them to the trash folder.
#+begin_src emacs-lisp
  (setq delete-by-moving-to-trash t)
#+end_src
~dired~ presents a bunch of details about each file that I don't file
particularly useful.  This makes it so ~dired~ only presents the name of
the file.
#+begin_src emacs-lisp
  (add-hook 'dired-mode-hook #'dired-hide-details-mode)
#+end_src
By default, ~dired~ shows hidden files.  By enabling ~dired-omit-mode~,
~dired~ we can hide them.
#+begin_src emacs-lisp
  (add-hook 'dired-mode-hook #'dired-omit-mode)
#+end_src
Sometimes we might want to see hidden files.  This makes it so that
pressing ~o~ reveals them.
#+begin_src emacs-lisp
  (keymap-set dired-mode-map "o" #'dired-omit-mode)
#+end_src
The files that I want to omit with ~dired-omit-mode~.
#+begin_src emacs-lisp
...
#+end_src
Disable "Omit N files" message that appears in the minibuffer after
enabling ~dired-omit-mode~.
#+begin_src emacs-lisp
  (setq dired-omit-verbose nil)
#+end_src
Pressing the ~b~ key moves you up the directory hierarchy.
#+begin_src emacs-lisp
  (keymap-set dired-mode-map "b" #'dired-up-directory)
#+end_src
Opening a lot of ~dired~ buffers can lead to extreme clutter your buffer
list.  This option makes it so that whenever you go into a new
directory from a ~dired~ buffer, Emacs kills the current ~dired~ buffer.
#+begin_src emacs-lisp
...
#+end_src
Adds icons to ~dired~ buffers for each file.  Note that you may need to
install extra fonts for this to work correctly.  For instructions on
how to do this, go to the GitHub for [[https://github.com/domtronn/all-the-icons.el][all-the-icons]].
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (setq mu4e-alert-icon "/home/tiago/.dot-files/email.png")
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defvar log-file
    (org-directory "albums.org.gpg"))

  (defvar papers-file
    (org-directory "papers.org.gpg"))
#+end_src
My agenda files:
#+begin_src emacs-lisp
  (setq org-agenda-file-regexp "\\`[^.].*\\.org\\\(\\.gpg\\\)?\\'")
  (setq org-agenda-files (list todo-file agenda-file log-file papers-file))
#+end_src
...
#+begin_src emacs-lisp
  (add-to-list 'org-agenda-custom-commands
  	     '("t" "List all PhD tasks" ((todo "TODO"))
  	       ((org-agenda-files (list todo-file)))))
#+end_src
Add menu item to list albums I haven't listened yet.
#+begin_src emacs-lisp
  (add-to-list 'org-agenda-custom-commands
  	     '("l" "List all unlistened albums" ((todo "TODO"))
  	       ((org-agenda-files (list log-file)))))
#+end_src
Add menu item to list unread papers
#+begin_src emacs-lisp
  (add-to-list 'org-agenda-custom-commands
  	     '("p" "List all unread papers" ((todo "TODO"))
  	       ((org-agenda-files (list papers-file)))))
#+end_src
...
#+begin_src emacs-lisp
  (start-process-shell-command
   "update" nil "cd $HOME/roam/daily && git pull")
#+end_src
List ~TODO~ items in column-view-mode
#+begin_src emacs-lisp
  (setq org-agenda-view-columns-initially t)
#+end_src
Do not list sub tasks
#+begin_src emacs-lisp
  (setq org-agenda-todo-list-sublevels nil)
#+end_src
In column view mode only show the name of the heading by default
#+begin_src emacs-lisp
  (setq org-columns-default-format "%70ITEM")
#+end_src
** Appearance
...
#+begin_src emacs-lisp
  (add-to-list 'org-babel-tangle-lang-exts '("rocq" . "v"))
  (add-to-list 'org-babel-tangle-lang-exts '("gospel" . "mli"))
  (add-to-list 'org-src-lang-modes '("gospel" . tuareg))
  (add-to-list 'org-src-lang-modes '("rocq" . coq))
#+end_src
Add "WONT-DO" to the todo tags.
#+begin_src emacs-lisp
  (setq org-todo-keywords
        '((sequence "TODO(t)" "|" "DONE(d!)" "WONT-DO(w@/!)")))
#+end_src
TODO tag colors
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (defun add-symbols ()
    ...
    (push '("#+begin_src gospel" . ?🛐) prettify-symbols-alist)
    (push '("#+begin_src rocq" . ?🦅) prettify-symbols-alist)
...
#+begin_src emacs-lisp
  (defun list-albums-by-artist ()
    (interactive)
    (with-current-buffer "art.org.gpg"
      (let* ((value (completing-read
        		   "Value: " (mapcar #'list (org-property-values "ARTIST")))))
        (unless (string-match "\\`{.*}\\'" value)
  	(setq value (concat "\"" value "\"")))
        (switch-to-buffer "art.org.gpg")
        (org-match-sparse-tree nil (concat "ARTIST=" value))
        )))
#+end_src
...
#+end_src
M-TAB expands the prefix of candidates (TAB completion).
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (marginalia-mode)
#+end_src
Keybindings for switch to buffer.
#+begin_src emacs-lisp
...
we can go to candidates in lines previous to the one we started in.
#+begin_src emacs-lisp
...
Command to compile a Typst project.
#+begin_src emacs-lisp
  (defun run-typst ()
    (interactive)
    (async-shell-command "typst watch main.typ" "Typst"))
#+end_src
...
#+begin_src emacs-lisp
  (use-package elfeed)
#+end_src
Update RSS on startup and every 2 hours.
#+begin_src emacs-lisp
  (elfeed-update)
...
#+begin_src emacs-lisp
  (setq elfeed-feeds
   '("https://rss.arxiv.org/atom/cs.PL"
     "https://www.worldpoliticsreview.com/feed/"
     "https://www.reddit.com/r/emacs.rss"
     "https://www.patreon.com/rss/chapotraphouse?auth=2TG4bQg76BCEXmj_RpOzlvax9O8TUffh"))
#+end_src
* Multimedia

Loads my music library onto EMMS (Emacs Multimedia System)
#+begin_src emacs-lisp
  (use-package emms)
  (emms-all)
  (emms-add-file "~/music")
...
Set ~MPV~ as my default music player.
#+begin_src emacs-lisp
  (setq emms-player-list '(emms-player-mpv))
#+end_src

Makes it so ~playerctl~ works with EMMS.
#+begin_src emacs-lisp
  (emms-mpris-enable)
#+end_src
...
#+begin_src emacs-lisp
  (defun delete-until-end ()
    (while (and (< (point) (- (point-max) 1))
  	      (not (and (eq (char-after) ?\?)
  			(eq (char-after (+ (point) 1)) ?*))))
      (delete-char 1))
    (delete-char 2))

  (defun delete-gospel-escape ()
    (beginning-of-buffer)
  ...
  (add-hook 'org-babel-post-tangle-hook #'delete-gospel-escape)
#+end_src
