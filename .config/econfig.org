...
#+begin_src emacs-lisp
  (defun epic () (setq gac-automatically-push-p t))

  (add-hook 'git-auto-commit-mode-hook #'epic)
#+end_src
...
#+begin_src emacs-lisp
  (setq org-export-buffer nil)
  (defun org-export-auto ()
    (when org-export-buffer
     (if (equal org-export-buffer 'beamer)
        (org-beamer-export-to-latex)
      (org-latex-export-to-latex))
    (org-babel-tangle)
    (let ((gospel (car (directory-files "." t "\\.mli$" t 1)))
  	(rocq (car (directory-files "." t "\\.v$" t 1))))
      (shell-command (concat "gospel check " gospel) "Gospel Type Checking")
      (shell-command (concat "rocq c " rocq ">> /dev/null") "Rocq Compilation"))))

  (add-hook 'after-save-hook #'org-export-auto)
#+end_src
...
#+begin_src emacs-lisp
  (defun latex-compilation-windows ()
    (interactive)
    (delete-other-windows)
    (switch-to-buffer "LaTeX")
    (my-split-window-right)
    (windmove-right)
    (switch-to-buffer "Gospel Type Checking")
    (split-window-below)
    (windmove-down)
    (switch-to-buffer "Rocq Compilation"))
#+end_src

Use the \gosp macro for code snippets
#+begin_src emacs-lisp
  (setq org-export-filter-code-functions
        '((lambda
  	  (s b info)
  	  (let ((s (replace-regexp-in-string "texttt" "gosp" s t t)))
  	    (replace-regexp-in-string "\\\\_" "_" s t t)))))
#+end_src
...
#+begin_src emacs-lisp
  (setq org-export-filter-body-functions
        '((lambda
  	  (s b info)
  	  (when (or (eq b 'latex) (eq b 'beamer))
  	   (let ((case-fold-search nil)
  		(macros '(("GospelSL" . "gospelsl")
  			  ("Gospel" . "gospel")
  			  ("Rocq" . "rocq")
  			  ("OCaml" . "ocaml")
  			  ("Peter" . "peter")
...
There is a new Wayland Emacs window manager.  It is superior to EXWM
in every way EXCEPT that I can't have multiple displays since it
cannot detect the correct GPU.  For now, I will leave these
configurations here in case this is fixed.
