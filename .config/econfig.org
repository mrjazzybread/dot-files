...
#+begin_src emacs-lisp :results none
  (start-process-shell-command
   "feh" nil
   "feh --bg-scale ~/.config/bg/lain.jpeg")
#+end_src
...
Kill the current buffer. For some reason I have to wrap the
~kill-this-buffer~ function with an ~interactive~ lambda
#+begin_src emacs-lisp :results none
...
Start a desktop application using /counsel linux app/
#+begin_src emacs-lisp :results none
...
#+begin_src emacs-lisp :results none
  (exwm-input-set-key (kbd "s-ESC") 'desktop-environment-lock-screen)
#+end_src
