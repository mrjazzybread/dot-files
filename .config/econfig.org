...
#+end_src
If the file is a pdf, open with ~zathura~, otherwise open normally with
~dired~.
#+begin_src emacs-lisp :results none
  (defun zathura-dired-find-file ()
    (interactive)
    (if (equal (file-name-extension (dired-get-filename)) "pdf")
        (zathura (dired-get-filename))
      (dired-find-file)))

  (add-hook 'dired-mode-hook
            (lambda () (local-set-key
                        (kbd "<return>") #'zathura-dired-find-file)))
#+end_src
...
#+begin_src emacs-lisp :results none
  (use-package hide-mode-line
    ...
    :hook (mu4e-headers-mode . hide-mode-line-mode)
    :hook (mu4e-dashboard-mode . hide-mode-line-mode)
    )
#+end_src
Pretty mode line
#+begin_src emacs-lisp :results none
