...
#+begin_src emacs-lisp :results none
  ...
  (bind-key "C-x g" 'magit-new-window)
#+end_src
* Appearance
...
#+begin_src emacs-lisp :results none
  (use-package proof-general
    ...
    :custom
    ;; when startin a proof, splits windows so that the goals  
...
#+end_src
Bookmark jump function that loads bookmark file
#+begin_src emacs-lisp :results none
  (defun eshell-jump (b)
    (interactive)
    (require 'bookmark)
...
#+begin_src emacs-lisp :results none
  ...
  (require 'eshell)
  (use-package eshell
    :hook (eshell-mode . eshell-change-appearance)
...
#+end_src
Always spawns eat on a new terminal
#+begin_src emacs-lisp :results none
  (defun multi-eat ()
    (interactive)
    (when (seq-find
           (lambda (val)
             (equal "*eat*" (buffer-name val)))
           (buffer-list))
...
#+begin_src emacs-lisp :results none
  ...
  #+end_src
This is a key binding that I use frequently in Org so I disable it in
Helm.
#+begin_src emacs-lisp :results none
  (require 'helm)
  (assq-delete-all (kbd "C-u") helm-map)
#+end_src
