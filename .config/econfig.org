...
#+end_src
Automatically export to latex on save (only for files name "main.org")
#+begin_src emacs-lisp :results none
  (setq export-org-buffer nil)
  (defun org-export-main ()
    (when export-org-buffer
      (org-beamer-export-to-latex)
      (org-babel-tangle)
      (let ((gospel (directory-files-recursively "." "\\.mli$"))
  	  (rocq (directory-files-recursively "." "\\.v$")))
        (dolist (v gospel) (shell-command (concat "gospel check " v) "Gospel Type Checking"))
        (dolist (v rocq) (shell-command (concat "rocq c " v ">> /dev/null") "Rocq Compilation")))))

  (add-hook 'after-save-hook #'org-export-main)
#+end_src
** Org Roam
